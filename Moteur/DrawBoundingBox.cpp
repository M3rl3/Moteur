...
void draw_bbox(sModelDrawInfo* model, unsigned int shaderID, glm::mat4 meshTransform)
{
    ...
    min_x = max_x = model->pVertices[0].x;
    ...
    for (int i = 0; i < model->vecVertices.size(); i++) {
        ...
        if (model->pVertices[i].y > max_y) max_y = model->pVertices[i].y;
        if (model->pVertices[i].z < min_z) min_z = model->pVertices[i].z;
        if (model->pVertices[i].z > max_z) max_z = model->pVertices[i].z;
    }
    ...
    // Apply the object's transformation matrix
    glm::mat4 matModel = meshTransform * transform;

    glUniformMatrix4fv(modelLocaction, 1, GL_FALSE, glm::value_ptr(matModel));
